...
/** Previous configuration before activating.
 Nil if active is NO;

 */
@property(nonatomic, strong, nullable) RTC_OBJC_TYPE(RTCAudioSessionConfiguration) *previousConfiguration;
@property(nonatomic, strong) NSMutableArray<RTC_OBJC_TYPE(RTCAudioSessionConfiguration) *> *configurationStack;

// Default value is YES
@property(nonatomic, assign) BOOL restoreSessionConfiguration;
 ...
 *  Each session activation should begin with conifigureSession:error: and end with unconfigureSession:
 *  |lockForConfiguration| must be called first.
 */
- (BOOL)configureSession:(RTC_OBJC_TYPE(RTCAudioSessionConfiguration) *)configuration
                   error:(NSError **)outError;

/** Restores the configuration to the current session.
 *  Each session activation should begin with conifigureSession:error: and end with unconfigureSession:
 *  |lockForConfiguration| must be called first.
 */
- (BOOL)unconfigureSession:(NSError **)outError;

/** Applies the configuration to the current session without it's activation.
 *  It is expected this method call may interrupt the ative audio session.
 *  Method is required to configure correctly CallKit audio session.
 *  Interruption context begin with configureInterruptionContext:error: and end with unconfigureInterruptionContext:
 *  |lockForConfiguration| must be called first.
 */
- (BOOL)configureInterruptionContext:(RTC_OBJC_TYPE(RTCAudioSessionConfiguration) *)configuration
                               error:(NSError **)outError;
 ...
 *  |lockForConfiguration| must be called first.
 */
- (BOOL)unconfigureInterruptionContext:(NSError **)outError;

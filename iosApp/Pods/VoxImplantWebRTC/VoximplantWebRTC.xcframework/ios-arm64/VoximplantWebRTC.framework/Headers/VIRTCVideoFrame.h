...
typedef NS_ENUM(NSInteger, RTCCamera) {
    RTCCameraUnspecified = AVCaptureDevicePositionUnspecified,
    RTCCameraBack = AVCaptureDevicePositionBack,
    RTCCameraFront = AVCaptureDevicePositionFront,
};
...
@property(nonatomic, readonly) RTCCamera camera;
...
/** Initializ an RTCVideoFrame from a frame buffer, rotation, camera, and timestamp.
 */
- (instancetype)initWithBuffer:(id<RTC_OBJC_TYPE(RTCVideoFrameBuffer)>)frameBuffer
                      rotation:(RTCVideoRotation)rotation
                        camera:(RTCCamera)camera
                   timeStampNs:(int64_t)timeStampNs;
...
- (void)renderToCVPixelBuffer:(CVPixelBufferRef)outputPixelBuffer;

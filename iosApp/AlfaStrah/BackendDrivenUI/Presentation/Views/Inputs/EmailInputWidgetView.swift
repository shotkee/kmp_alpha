...
extension BDUI {
	class EmailInputWidgetView: WidgetView<EmailInputWidgetDTO> {
		private var userInputView: UserSingleLineInputView?
		
		required override init(
			block: EmailInputWidgetDTO,
			...
			self.userInputView?.validationRules.append(EmailValidationRule())
			
			self.userInputView?.validateAsYouType = false
			self.userInputView?.textField.keyboardType = .emailAddress
			self.userInputView?.textField.autocapitalizationType = .none
